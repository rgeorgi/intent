...
split_p.add_argument('FILE', nargs='+',
                     help='XIGT files to gather together in order to generate the train/dev/test split', type=globfiles)
split_p.add_argument('--train', default=0.8, help='The proportion of the data to set aside for training.',
                     type=proportion)
...
filter_p.add_argument('--require-lang', help='Require instances to have language line', action='store_true',
                      default=False)
filter_p.add_argument('--require-gloss', help='Require instances to have gloss line', action='store_true',
                      default=False)
filter_p.add_argument('--require-trans', help='Require instances to have trans line', action='store_true',
                      default=False)
filter_p.add_argument('--require-gloss-pos', help='Require instance to have gloss pos tags', action='store_true',
                      default=False)
filter_p.add_argument('--require-grammatical', help='Filter out ungrammatical instances', action='store_true',
                      default=False)
filter_p.add_argument('--max-instances', help='Limit the number of output instances', default=0, type=int)
filter_p.add_argument('--require-aln', help='Require instances to have 1-to-1 gloss/lang alignment.',
...
extract_p.add_argument("--tagmap", dest='tagmap', help='Provide a tagset mapping to convert POS tags for this file.',
                       type=existsfile)
extract_p.add_argument('--classifier-prefix', dest='classifier_prefix',
                       help='Output prefix for gloss-line classifier (No extension).', default=None)
extract_p.add_argument('--classifier-feats', dest='classifier_feats',
                       help='Features to use for training the classifier. ' + str(CLASS_FEATS_ALL),
                       default=CLASS_FEATS_DEFAULT, type=csv_choices(CLASS_FEATS_ALL))
extract_p.add_argument('--tagger-prefix', dest="tagger_prefix", help='Output prefix for lang-line tagger.',
                       default=None)
...
extract_p.add_argument('--sent-type', dest='sent_type', choices=[SENT_TYPE_T_G, SENT_TYPE_T_L],
                       help="Choose between translation-gloss and translation-lang", default=SENT_TYPE_T_L)
extract_p.add_argument('--no-alignment-heur', action='store_true',
                       help='Disable adding heuristic alignment results to aligned sentences.', default=False)
...
eval_p.add_argument('--classifier-feats', dest='classifier_feats',
                    help='Features to use for training the classifier. ' + str(CLASS_FEATS_ALL),
                    default=CLASS_FEATS_DEFAULT, type=csv_choices(CLASS_FEATS_ALL))
eval_p.add_argument('--ds-projection',
                    help='Evaluate DS projection methods against the gold standard DS provided in the file',
                    action='store_true', default=False)
eval_p.add_argument('--pos-projection',
                    help='Evaluate POS projection method against gold standard POS tags in the file.',
                    action='store_true', default=False)
eval_p.add_argument('--alignment', help='Test alignment methods against the alignment provided in the file.',
                    action='store_true', default=False)
eval_p.add_argument('--pos-tagger', help='Test a pos tagger on the language line (requires language line POS tags)',
                    default=None, type=existsfile)
eval_p.add_argument('--tagmap-gold', help='Use this tag mapping on gold tags', default=None, type=existsfile)
eval_p.add_argument('--tagmap-trans', help='Use this tag mapping on translation-line tags', default=None,
                    type=existsfile)
eval_p.add_argument('--output', '-o', help='Redirect output to this path.')
...
# ===============================================================================
project_p = register_subparser(CMD_PROJECT,
                               help="Command that will (re)project pos/ps/ds using the specified pos source and alignment type.")
...
project_p.add_argument('--aln-method', dest='aln_method',
                       choices=ARG_ALN_METHODS_ALL, help="The alignment method to use for projection.",
                       default=ARG_ALN_ANY)
project_p.add_argument('--completeness', dest='completeness',
                       type=float, default=0,
                       help="Ratio of words which must be aligned in order to project an instance.")
